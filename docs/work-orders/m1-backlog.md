...
**Status:** Approved — 2026-07-24. Execution authorized under [ADP-001](../implementation-plans/ADP-001-m1-working-pipe-and-trim.md). **Progress (2026-07-24, synthetic fixtures, local build only):** WO-100–106 ✅ · WO-107–112 ✅ · **WO-113 ✅** (all privacy/security guards, each proven to fail when violated) · **WO-114 ◑** (all synthetic-runnable §10 checks pass through the API; the real-50-clip-day and Apple-Memory checks are owner-gated). **67 passed / 1 skipped; frontend typechecks & builds clean. Every M1 code lane is done.** What's left needs the owner: **real footage + a recorded ADR-002 consent** for the two remaining ES-001 §10 exit gates, and authorization to **push** (the branch is well ahead of the public `origin`, unpushed).**
...
- **Status:** ✅ **Complete — 2026-07-24.** `backend/api/`: all 13 §6 endpoints; **ADR-011 security** (Host + Origin allow-list, per-launch capability token on state-changing routes, no CORS middleware so no wildcard ACAO, path-scrubbed error envelope); in-memory threaded job runner (`{state, progress, error}`, failing jobs surface a scrubbed error); range-capable proxy/draft/download serving. `analyze`/`propose` return 501 until WO-111/112. Codes against the WO-101 `Protocol`s. Bound to `127.0.0.1` in `run.py`. Tests green.
...
- **Status:** ✅ **Complete against synthetic fixtures — 2026-07-24.** `backend/analysis/`: OpenCV per-second sharpness (Laplacian var), exposure clipping, shake (phaseCorrelate global shift), motion energy, and scene cuts from the **proxy**; audio RMS from the **original** (read-only, since proxies are silent) via an ffmpeg PCM pass. All signals **normalised 0..1** so the proposer's fixed floors apply (reference constants are config). `people_count` stays `null` (M2). `FileAnalysisStore` persists `analysis.json`. Tests green. **Manifest note:** uses `opencv-python-headless` (server-side, no GUI) rather than the declared `opencv-python`. **Deferred:** the real 50-clip-day timing gate (needs consent); cross-clip perceptual-hash `dup_group` (not needed for M1 trim, left `null`).
...
- **Status:** ✅ **Complete — 2026-07-24.** `backend/propose/`: deterministic §5.2 rule engine (pure logic over `Analysis`) — longest good window not crossing a scene cut, leading/trailing quality + static-dead-air trim, universal 1.0 s floor, **full-clip fallback when nothing clears the floors**. Emits one `ReasonRecord` per factor with `evidence_refs` into the signals that drove it (ADR-006). Wired into `POST /api/analyze` + `POST /api/propose/trim` (applies the proposal as the pending effective trim, origin→`proposed`). 8 rule tests + analyze→propose integration green. Demonstrated on real pixels: a blurry opening → *"Trimmed the first 1 s — too blurry to keep (blur 0.01 vs 0.35 floor)."*
...
- **Status:** ✅ **Complete — 2026-07-24.** All guards in `tests/guards/`, **each proven to fail when violated**: ADR-011 security (cross-origin POST rejected · missing/invalid capability token rejected · no wildcard CORS · error envelope carries no absolute path); repo-hygiene (`project.json` gitignored anywhere · only `fixtures/synthetic/` tracked, ADR-013 · no `madmom`, ADR-003); read-only originals (via WO-102); **frontend ships no remote assets** (no CDN/fonts/remote imports/fetches — `xmlns` namespace URIs correctly allowed); **media backend imports no network libraries** (originals never leave the device).
...
- **Status:** ◑ **Synthetic-runnable checks complete — 2026-07-24.** `tests/integration/test_full_flow_api.py` runs the full M1 flow through the HTTP API and asserts the ES-001 §10 checks that don't need real footage: §10.2 byte-equivalent reopen · §10.3 no writes beneath `media_root` · §10.4 every included clip proposed with a readable reason · §10.5 curation + trim overrides (exclude/restore, delete/restore, adjust→`adjusted`, remove→`dismissed`, proposals retained) round-trip and survive reload · §10.6 all three audio modes pass QA · §10.7 editing after a finalize approval resets it (store now enforces the §7 invariant). **Owner-gated (skipped with reason):** §10.1 a real ~50-clip day and §10.8 the Apple Photos Memory comparison — need the owner's real footage + a recorded ADR-002 consent.
