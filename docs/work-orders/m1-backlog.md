...
- **Status:** ◑ **Partial — 2026-07-24.** Done (`tests/guards/`): **ADR-011 security guards** — cross-origin POST rejected, missing/invalid capability token rejected, no wildcard CORS, error envelope carries no absolute path; **repo-hygiene gates** — `project.json` gitignored anywhere, only `fixtures/synthetic/` tracked (ADR-013), no `madmom` (ADR-003); **read-only originals** (via WO-102). **Deferred to WO-107+:** the frontend-bundle no-remote-assets check and the media-path egress check (need the real frontend / a running server).
