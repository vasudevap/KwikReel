...
**Still documents only — no code and no media exist.** Nothing may be built until the owner approves the backlog and authorizes an ADP. When that happens, **the first work is WO-100: a clickable prototype with fake data** (ADR-008), not backend code. Read [handoff.md](handoff.md) before assuming anything exists.
